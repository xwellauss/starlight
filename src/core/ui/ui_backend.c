...
static void push_rect(float x0, float y0, float x1, float y1, vec4s color, vec2s rect_pos, vec2s rect_size, vec4s corner_radius, vec4s border_width)
{
...
void ui_backend_init()
{
...

	{
		...
		vertex_buffer_init_with_layout(&rect_vertex_buffer, NULL, MAX_RECTS * sizeof(RectQuad), rect_indices, sizeof(rect_indices), true, layout);
	...
	for(int i = 0; i < cmds.length; i++)
	{
		...
		case CLAY_RENDER_COMMAND_TYPE_IMAGE:
		{
			...
			if(is_image)
			{
				...
				push_rect(x0, y0, x1, y1, color, rect_pos, rect_size, corner_radius, border_width);	
				shader_bind(&rect_shader);
