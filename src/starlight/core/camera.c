...
void move_camera(Camera* camera, InputState input_state)
{
	...
	if(camera->camera_type & CAMERA_LOOK_AROUND)
	{
		...
		clicking = clicking && (mouse_position.x >= window_width/2.0f);
#endif
		window_set_cursor_mode(clicking ? CURSOR_MODE_DISABLED : CURSOR_MODE_NORMAL);
